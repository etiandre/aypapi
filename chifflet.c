...
// than using offsets.
// maybe structs ?
void calculate_meters(long long cpu_val[], long long uncore_val[],
                      double dt,
                      double meters[],
                      int n_uncores,
                      int n_cpus_per_uncore)
{
  ...
  for (int u_idx=0; u_idx<n_uncores; u_idx++) {
    ...
    meters[meters_off+METER_FLOPS] = flop * pow(10,-6) / dt; // [MFLOP count] / [s]
    meters[meters_off+METER_MEM_BW] = datavolume * pow(10,-6) / dt; // [MB] / [s] (base 10)
    meters[meters_off+METER_PKG_PWR] = cpu_nrj * pow(2,-32) / dt; // [J] / [s] = [W]
