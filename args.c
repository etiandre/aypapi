...
static struct argp_option options[] = {
                                       {"sleeptime", 't', "time", 0, "Time in seconds between two measurments.", 0},
                                       {"sockets", 's', "uncore_list", 0, "Comma-separated list of sockets / package indexes to meter.", 0},
...
static error_t parse_opt(int key, char *arg, struct argp_state *state) {
    ...
    case 't': {
      double x = atof(arg);
      if (x <= 0.0) DIE("Invalid value for -t\n");
